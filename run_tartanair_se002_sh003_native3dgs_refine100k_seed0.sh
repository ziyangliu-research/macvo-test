...
# Remaining six TartanAir Stereo Challenge sequences: SE002, SE003, SH000-SH003.
# Online protocol: strict8:2, W20/rho=.30/B100/M50/Th=.10, serial/default ReSplat stream.
# Online endpoint: save every held-out test GT/render pair.
...
# No CUDA-watchdog preflight is performed by this launcher.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/tartanair_se002_sh003_native3dgs_refine100k}"
...
RUNNER="run_pipeline_execution_benchmark_repro_tartanair_native3dgs_lpips.py"
...
PY
  ...
  PIPELINE_SAVE_ONLINE_TEST_RENDERS=1 \
  ...
  if [[ "$status" -ne 0 ]]; then
    ...
    python summarize_tartanair_se002_sh003_native3dgs_all.py "$OUTPUT_ROOT" "$SEED" || true
    exit "$status"
  fi
